...
var plugin_dialog = {
    ...
    add: function (core) {
      ...
      core.addModule([window.SUNEDITOR_MODULES.dialog]);
      ...
      context.customLink.focusElement = link_dialog.querySelector(
        "._se_link_url"
      );
      context.customLink.linkAnchorText = link_dialog.querySelector(
        "._se_link_text"
      );
      context.customLink.targetSelect = link_dialog.querySelector(
        ".se-input-select"
      );
    ...
    submit: function (e) {
      ...
      const submitAction = function () {
        ...
        if (!this.context.dialog.updateModal) {
          ...
          oA.href = url;
          // Trim space step 1
          oA.textContent = anchorText.trim();
          ...
          if (selectedFormats.length > 1) {
            ...
            // Trim step 2: reduce selection range: https://github.com/JiHong88/SunEditor/issues/719#issuecomment-826122741
            const range = this.getRange();
            const ls = range.startContainer.textContent
              .substr(range.startOffset)
              .match(/^\s+/);
            const rs = range.endContainer.textContent
              .substr(0, range.endOffset)
              .match(/\s+$/);
            this.setRange(
              range.startContainer,
              ls ? range.startOffset + ls[0].length : range.startOffset,
              range.endContainer,
              rs ? range.endOffset - rs[0].length : range.endOffset
            );
            this.insertNode(oA);
            console.log("insertNode called");
          }
          ...
          console.log("This setRange in 'else' is not called.");
        }
